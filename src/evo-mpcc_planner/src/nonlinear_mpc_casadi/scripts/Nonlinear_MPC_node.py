...
class MPCKinematicNode:
    def __init__(self):
        ...
        self.is_ot = self.parameters_algo['overtaking']
        self.vel_scale = self.parameters_algo['vel_scale']
        ...
        if self.is_ot:
            ...
            obstacle_pose = []
        self.obstacle_pose = np.array(obstacle_pose)
        rospy.loginfo("static obstacle poses are: %s", self.obstacle_pose)
        self.obstacle_pub = rospy.Publisher('/static_obstacles', MarkerArray, queue_size=10)
        for i in range(5):
            self.publish_obstacles(self.obstacle_pose, self.obstacle_pub)
            rospy.sleep(0.2)
        ...
        # MPC related initializations
        self.mpc = MPC("EVO-MPCC", "cimpcc_model")
    ...
    def publish_obstacles(self, obstacle_poses, obstacle_pub):
    ...
    def initialize_MPC(self):
        ...
        self.mpc.set_initial_params(self.param, self.vheid, self.is_ot)
    ...
    def read_waypoints_array_from_csv(self, filename, type="path"):
        ...
        if type == "traj":
            ...
            v_points = np.array([float(point[2])*self.vel_scale for point in path_points])
    ...
    def controlLoopCB(self, event):
        ...
        if self.goal_received == True and self.is_BO_trainning == False:
            ...
            if self.other_planner == "pp":
                ...
                mpc_time = time.time()
                ##################################### main planner #####################################
                first_control, trajectory, control_inputs, opti_value = self.mpc.solve(current_state, self.obstacle_pose)
                ##################################### main planner #####################################
